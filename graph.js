...
const getDataMinsAndMaxes = (allPropertyPoints) => {
  ...
  for (const key of keys) {
    let yValues = [];
    let xValues = [];
      for(i = 0; i < allPropertyPoints[key].length; i++){
...
const updateValueReadout = (seekerData) => {
  ...
  Object.keys(seekerData).forEach(function (property) {
    valueReadout = document.getElementById(property + 'valueReadout');
...
const mouseYToZoomLevel = (newY, originalY, originalZoom) => {
  ...
  zoomLevel = originalZoom + (yDistance * zoomRatio) ;
...
overviewCanvas.addEventListener('mousedown', (event) => {
  ...
  sliderWindowRect = overviewSliderStart.getBoundingClientRect();
  overviewRect = overviewCanvas.getBoundingClientRect();
  sliderWindowWidth = sliderWindowRect.right - sliderWindowRect.left;
  overviewWidth = overviewRect.right - overviewRect.left;
...
canvases = createCanvases(nonTimeProperties);
